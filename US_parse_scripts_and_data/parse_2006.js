// for http://oas.samhsa.gov/2k6state/AppB.htm
var data = [];
var year = 2006;
for (var j = 2; j < 53; j++) {	
	var state = {};
	...
	state.cannabis=[];
	for (var i = 0; i < 3; i++) {
		var age_str = "/html/body/div[3]/table/thead/tr[2]/th["+(i+1)+"]";
	    var age_group = $x(age_str)[0].innerHTML;
		var value_str = "/html/body/div[3]/table/tbody/tr["+j+"]/td["+(4+i*2)+"]";
	...
	state.alcohol=[];
	for (var i = 0; i < 3; i++) {
		var age_str = "/html/body/div[10]/table/thead/tr[2]/th["+(i+1)+"]";
	    var age_group = $x(age_str)[0].innerHTML;
		var value_str = "/html/body/div[10]/table/tbody/tr["+j+"]/td["+(4+i*2)+"]";
		var value = parseFloat($x(value_str)[0].innerHTML);
		var ob = {};
		ob.year = year;
		if (age_group.indexOf(" or Older") != -1) {
